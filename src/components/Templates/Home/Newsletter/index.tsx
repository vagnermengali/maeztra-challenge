    ...
    <section className="bg-brand9 py-6 border-t-2 border-brand8 border-solid pt-5 md:pt-[39px] -z-10">
      <div className="container flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-14">
        ...
        <div className="flex flex-col gap-2 sm:gap-1">
          <form
            className="flex items-center w-full gap-[15px] sm:gap-0 sm:w-[280px] md:w-[410px] lg:w-[630px] relative"
          ...
          >
            ...
            <button
              className={`${
                submitted ? "bg-green-500" : "bg-brand1"
              } max-w-full w-[69px] sm:w-[131px] rounded-[4px] hover:text-brand4 text-brand10 font-bold text-sm sm:text-base leading-5 sm:leading-6 h-[26px] sm:h-10 transition-all duration-500 ease-in-out`}
            >
              {submitted ? "Enviado" : "Enviar"}
            </button>
          </form>
          {!validEmail && (
            <p className="text-red-500 text-xs max-auto sm:mx-0 text-center">
              Por favor, insira um endereço de e-mail válido.
            </p>
          )}
          {submitted && (
            <p className="text-green-500 text-xs max-auto sm:mx-0 text-center">
              Obrigado pela inscrição! Entraremos em contato.
            </p>
          )}
        </div>
      </div>
