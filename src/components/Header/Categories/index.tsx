...
const Categories = ({
  ...
  return (
    <div
      className="bg-brand10 hidden relative lg:block"
    ...
    >
      <div className="container flex items-center justify-center gap-[15px]">
        <Link
          ...
          title="Novidades"
        >
          <Icon id="Dress" width="12.5" height="15" /> Novidades
        </Link>
        {MenuLink.map((menu, index) => (
          <div key={index}>
            ...
            {activeMenu === index && (
              <div
                className="absolute bg-white w-full left-0 right-0 top-10 shadow-lg py-10 px-4 mt-2 rounded-lg"
                ...
                <div className="container relative flex flex-col gap-4">
                  {menu.links.map((link, idx) => (
                    <Link
                      ...
                      className={`${
                        link.label == "Ver tudo"
                          ? "absolute bottom-0 right-4 underline font-bold uppercase "
